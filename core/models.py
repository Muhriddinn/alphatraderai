...
class MarketAlert:
    ...
    # Strategy fields (set by alert_engine)
    strategy_direction: Optional[str] = None  # "LONG" or "SHORT"
    strategy_entry: float = 0.0
    strategy_sl: float = 0.0
    strategy_tp: float = 0.0
    strategy_reason: str = ""

    # Real liq clusters from aggregator
    liq_clusters: list = field(default_factory=list)

    # CEXTrack-style activity text
    activity_text: str = ""

    # Extra dynamic attrs (set via __post_init__)
    extra: dict = field(default_factory=dict)
    cvd_data: dict = field(default_factory=dict)
