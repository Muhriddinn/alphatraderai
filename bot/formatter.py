...
def build_short_signal(
    ...
    if w:
        ...
        if vol_24h <= 0:
            vol_24h = getattr(w, "volume_24h", 0) or 0
        vol_pct = (w.volume_usdt / vol_24h * 100) if vol_24h > 0 else 0

        # 💰 buy / 💸 sell / 🤔 activity
        if w.direction and w.direction.value == "neutral":
            emoji = "🤔"
            dir_word = "activity"
        elif w.buy_volume > w.sell_volume:
            emoji = "💰"
            dir_word = "buying"
        elif w.sell_volume > w.buy_volume:
            emoji = "💸"
            dir_word = "selling"
        else:
            emoji = "🤔"
        ...
        exchange_name = "Binance Futures" if "binance" in str(w.exchange).lower() else str(w.exchange).replace("Exchange.", "")
        price_arrow = "⬆️" if w.price_change_pct >= 0 else "⬇️"
        elapsed = getattr(w, "duration_seconds", 0) or 0

        # CEXTrack format — exactly as requested
        symbol_short = alert.symbol.replace('USDT', '')
        lines.append(f"#{symbol_short} {dir_word} {emoji} {fmt_usdt(w.volume_usdt)} in {fmt_dur(elapsed)} ({vol_pct:.0f}%) on {exchange_name}")
        lines.append(f"P: {fmt_price(w.volume_usdt / w.token_qty if w.token_qty > 0 else 0)} {price_arrow} ({w.price_change_pct:+.2f}%)")
        lines.append(f"Vol 24h: {fmt_usdt(vol_24h)}")

        # Last seen
        from db.whale_db import get_last_whale_info, fmt_last_seen
        last_info = get_last_whale_info(alert.symbol)
        if last_info and last_info["created_at"]:
            ago_str = fmt_last_seen(last_info["created_at"])
            lines.append(f"Last {ago_str}")
        ...
        emoji = "💰" if buy_v >= sell_v else "💸"
        bias = "buying" if buy_v >= sell_v else "selling"
        token_qty = extra.get("whale_token_qty", 0) or 0
        lines.append(f"💰 <b>#{alert.symbol.replace('USDT','')}</b> {emoji} <b>{bias}</b>")
        lines.append(f"  {fmt_usdt(total_w)}")
        vol_24h = extra.get("volume_24h", 0) or 0
        if vol_24h > 0:
            vol_pct = total_w / vol_24h * 100
            lines.append(f"  Vol 24h: {fmt_usdt(vol_24h)} ({vol_pct:.0f}%)")
    elif extra.get("last_whale_usdt", 0) > 0:
        lw_usdt = extra["last_whale_usdt"]
        lw_side = extra.get("last_whale_side", "buy")
        emoji = "💰" if lw_side == "buy" else "💸"
        dir_word = "buying" if lw_side == "buy" else "selling"
        lines.append(f"💰 <b>#{alert.symbol.replace('USDT','')}</b> {emoji} <b>{dir_word}</b>")
        lines.append(f"  {fmt_usdt(lw_usdt)}")
        if lw_ago > 0:
    ...
    cvd_1m = (cvd_data.get("cvd_1m", 0) if cvd_data else 0) or extra.get("cvd_1m", 0) or 0
    cvd_5m = (cvd_data.get("cvd_5m", 0) if cvd_data else 0) or extra.get("cvd_5m", 0) or 0
    cvd_15m = (cvd_data.get("cvd_15m", 0) if cvd_data else 0) or extra.get("cvd_15m", 0) or 0
    cvd_trend = (cvd_data.get("cvd_trend", "flat") if cvd_data else "flat") or "flat"
...
def build_full_signal(
    ...
    if w:
        ...
        symbol_short = alert.symbol.replace('USDT', '')
        lines.append(f"#{symbol_short} {dir_word} {emoji} {fmt_usdt(w.volume_usdt)} in {fmt_dur(elapsed)} ({vol_pct:.0f}%) on {exchange_name}")
        lines.append(f"P: {fmt_price(w.volume_usdt / token_qty if token_qty > 0 else 0)} {price_arrow} ({w.price_change_pct:+.2f}%)")
        lines.append(f"Vol 24h: {fmt_usdt(vol_24h)}")
    ...
    # ── MULTI-TF ANALYSIS ────────────────────────────
    c5m_pct = extra.get("price_change_5m", 0) or 0
    c15m_pct = extra.get("price_change_15m", 0) or 0
    c1h_pct = extra.get("price_change_1h", 0) or 0
    c4h_pct = extra.get("price_change_4h", 0) or 0
    c1d_pct = extra.get("price_change_1d", 0) or 0

    tf_parts = []
    if c5m_pct != 0:
        tf_parts.append(f"5m: {_pct_color(c5m_pct)} {c5m_pct:+.2f}%")
    if c15m_pct != 0:
        tf_parts.append(f"15m: {_pct_color(c15m_pct)} {c15m_pct:+.2f}%")
    if c1h_pct != 0:
        tf_parts.append(f"1h: {_pct_color(c1h_pct)} {c1h_pct:+.2f}%")
    if c4h_pct != 0:
        tf_parts.append(f"4h: {_pct_color(c4h_pct)} {c4h_pct:+.2f}%")
    if c1d_pct != 0:
        tf_parts.append(f"1d: {_pct_color(c1d_pct)} {c1d_pct:+.2f}%")
    ...
    # ── VOLUME PROFILE (agar mavjud bo'lsa) ──────────
...
def build_live_pnl_update(
    ...
    if whale_usdt > 0:
        ...
        base_sym = signal.symbol.replace("USDT", "")
...
def build_bookmap_message(
    ...
    sr = sr_levels or {"supports": [], "resistances": []}

    # Top 3 buy/sell devorlar
    ...
    # Imbalance
    imbalance = total_buy / total_sell if total_sell > 0 else 99
    ...
    # Imbalance bar
    if imbalance >= 1:
        bar_buy = min(int(imbalance * 2), 10)
        bar_sell = 10 - bar_buy
        bar = "█" * bar_buy + "░" * bar_sell
        bias = f"🟢 BUY devor kuchli ({imbalance:.1f}x)"
    else:
        bar_buy = 10 - min(int((1/imbalance) * 2), 10)
        bar_sell = 10 - bar_buy
        bar = "█" * bar_buy + "░" * bar_sell
        bias = f"🔴 SELL devor kuchli ({1/imbalance:.1f}x)"

    lines.append(f"  🟢 [{bar}] 🔴")
    lines.append(f"  📊 Imbalance: {imbalance:.2f} — {bias}")
    lines.append("")

    # BUY devorlar (3 ta eng katta)
    if top_buy:
        lines.append("🟢 <b>BUY DEVORLAR (3 ta eng katta):</b>")
        for i, w in enumerate(top_buy, 1):
            dist = (current_price - w["price"]) / current_price * 100 if current_price > 0 else 0
    ...
    # SELL devorlar (3 ta eng katta)
    if top_sell:
        lines.append("🔴 <b>SELL DEVORLAR (3 ta eng katta):</b>")
        for i, w in enumerate(top_sell, 1):
            dist = (w["price"] - current_price) / current_price * 100 if current_price > 0 else 0
            duration_str = _fmt_duration(w.get("first_seen_ts", 0))
...
def build_showmore_keyboard(signal_id: str) -> list:
    """Keyboard for expanded view — with Qisqartirish button and VP timeframe buttons"""
    return [
        [
            {"text": "📊 VP 5m", "callback_data": f"vp_tf:{signal_id}:5m"},
            {"text": "📊 VP 15m", "callback_data": f"vp_tf:{signal_id}:15m"},
            {"text": "📊 VP 1h", "callback_data": f"vp_tf:{signal_id}:1h"},
            {"text": "📊 VP 4h", "callback_data": f"vp_tf:{signal_id}:4h"},
            {"text": "📊 VP 1d", "callback_data": f"vp_tf:{signal_id}:1d"},
        ],
        [
