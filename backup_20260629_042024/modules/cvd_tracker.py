...
class CVDTracker:
    ...
    def get_cvd_data(self, symbol: str) -> dict:
        ...
        Qaytaradi:
        {
            "cvd_current": 1234567.0,    # Hozirgi CVD
            "cvd_1m": +234567.0,         # 1 daqiqadagi o'zgarish
            "cvd_5m": -567890.0,         # 5 daqiqadagi o'zgarish
            "cvd_direction": "bullish",  # bullish/bearish
            "buy_volume_1m": 1234567.0,
            "sell_volume_1m": 987654.0,
        }
        """
        history = self._cvd_history.get(symbol, [])
        if not history:
            return {
                "cvd_current": 0, "cvd_1m": 0, "cvd_5m": 0,
