...
logger.add("logs/crypto_monitor.log", rotation="50 MB", retention="7 days", level="DEBUG")
...
class CryptoMonitorApp:
    ...
    async def _on_symbols_ready(self, symbols: list[str]):
        """Connector symbollar ro'yxatini olganda ob_tracker va digest ga beradi"""
        logger.info(f"📋 {len(symbols)} symbol tayyor → OB tracker va Digest yangilanmoqda")
    ...
    async def start(self):
        ...
        await asyncio.sleep(_rnd.uniform(5, 25))
        ...
        await asyncio.sleep(_rnd.uniform(3, 10))
