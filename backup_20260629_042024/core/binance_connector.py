...
class BinanceFuturesConnector:
    ...
    async def _poll_open_interest(self):
        """Poll OI for all symbols every 10 seconds via REST"""
        while self._running:
            try:
                tasks = [
                    self._fetch_oi(symbol)
                    for symbol in self.symbols[:500]
                ]
    ...
    async def _load_volume_baselines(self):
        ...
        # Batch: 5 ta coin ni parallel yuklaymiz (avval 10 edi — IP ban muammosi)
        # Har bir symbol ~27 weight (ticker 5 + premiumIndex 10 + OI 1 + klines×5 10 + depth 1)
        # 5 symbol/batch = 135 weight/burst. 4s delay = ~2000 weight/min < 2400 limit
        batch_size = 5
        ...
        logger.info(f"✅ Bootstrap tugadi: {loaded}/{total} symbol")
