    ...
    async def _get_extra_data(self, exchange: str, symbol: str) -> dict:
        ...
        if not extra["ob_buy_walls"] and not extra["ob_sell_walls"] and price > 0:
            try:
                ...
                logger.debug(f"OB REST fallback error {symbol}: {e}")
    ...
    def _calculate_strategy(
        ...
        if oi_ev:
            if oi_ev.change_pct > 1:
                # OI up + price up = bullish, OI up + price down = bearish
                price_chg = extra.get("price_change_5m", 0)
                if price_chg > 0:
                    bullish_score += 2
                    reasons.append(f"OI +{oi_ev.change_pct:.1f}% (narrative UP)")
                else:
                    bearish_score += 2
                    reasons.append(f"OI +{oi_ev.change_pct:.1f}% (narrative DOWN)")
            elif oi_ev.change_pct < -1:
                bullish_score += 1
                reasons.append(f"OI {oi_ev.change_pct:.1f}% (positions closing)")
